...
private:
    /* data */
    rclcpp::Publisher<rm_serial_driver_nav_msgs::msg::FromSentry>::SharedPtr from_sentry_pub_;
    uint16_t time_;
    uint16_t hp_;
    uint8_t mode_;
...
{
    time_ = 300;
    from_sentry_pub_ = this->create_publisher<rm_serial_driver_nav_msgs::msg::FromSentry>("/from_sentry", 10);
    RCLCPP_INFO(this->get_logger(), "sim_pub has been started");
    auto msg = rm_serial_driver_nav_msgs::msg::FromSentry();
